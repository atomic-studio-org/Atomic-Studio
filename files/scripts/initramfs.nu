...
run-external '/usr/libexec/rpm-ostree/wrapped/dracut' '--no-hostonly' '--kver' $"($QUALIFIED_KERNEL)" '--reproducible' '-v' '--add' 'ostree' '-f' $"/lib/modules/($QUALIFIED_KERNEL)/initramfs.img"

run-external 'chmod' '0600' $"/lib/modules/($QUALIFIED_KERNEL)/initramfs.img"
